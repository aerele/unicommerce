  ...
  refresh(frm) {
    ...
    sync_buttons.forEach((action) => {
      frm.add_custom_button(
        ...
        () => {
          frappe.call({
            method: "unicommerce.unicommerce.utils.force_sync",
            args: {
              document: action,
            },
            callback: (r) => {
      ...
      () => {
        frappe.prompt(
          ...
          (values) => {
            ...
            const inclusive_days = frappe.datetime.get_diff(to, from) + 1;
            if (inclusive_days > max_days) {
              frappe.msgprint(
                __("Date range cannot be longer than {0} days.", [max_days]),
              );
              return;
            }
            frappe.call({
              method:
                "unicommerce.unicommerce.sync_old_orders.enqueue_sync_old_orders",
              args: {
